...
export async function callRpc<T = unknown>(fn: string, args?: Record<string, unknown>): Promise<T> {
  // Call via `any` to avoid tight overload typing on `supabase.rpc` while
  // keeping runtime behavior unchanged. Replace with a more specific
  // typing if/when `src/types/database.ts` is fully generated.
