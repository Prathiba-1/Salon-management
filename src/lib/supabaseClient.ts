 ...
 *
 * Uses `any` cast deliberately: `src/types/database.ts` is a placeholder
 * (real types generated by `npm run gen-types` after M7 migrations are
 * applied to a live project). Until the real types exist, the generated
 * RPC parameter types are `never`, which blocks every call site. The cast
 * is isolated to this single wrapper — nowhere else in the codebase calls
